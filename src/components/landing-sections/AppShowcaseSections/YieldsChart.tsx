...
export default function YieldsSection() {
  const [selectedDataPoint, setSelectedDataPoint] =
    useState<YieldDataPoint | null>(null);
  const [timeRange, setTimeRange] = useState<TimeRange>("1y");

  const filteredData = extendedYieldData.slice(
  ...
  return (
    <Card className="col-span-1 md:col-span-2">
      <CardHeader>
        ...
        <CardDescription>
          Look at how your yields have performed compared to average trends
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          >
            <defs>
              <linearGradient id="colorYield" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor={chartConfig.yield.color}
                  stopOpacity={0.8}
                />
                <stop
                  offset="95%"
            ...
            <Area
              type="monotone"
              dataKey="yield"
              stroke={chartConfig.yield.color}
              fillOpacity={1}
              fill="url(#colorYield)"
            />
            <Line
        ...
        {selectedDataPoint && (
          <div className="mt-4 p-4 bg-muted rounded-md">
            <h4 className="text-lg font-semibold mb-2">
              Selected Month: {selectedDataPoint.month}
            </h4>
